...
export function registerMissionRoutes(app: any, progressTracker: ProgressTracker): void {
  // POST /api/v1/missions - Create new mission
  app.post('/api/v1/missions', async (request: Request) => {
    try {
      const body = await request.json();
      const { title, description, tasks, metadata } = body as any;
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create mission' }), 
  ...
  app.get('/api/v1/missions', async (request: Request) => {
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to retrieve missions' }), 
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
  });
  ...
  app.get('/api/v1/missions/:id', async (request: Request, params: { id: string }) => {
    try {
      const { id } = params;
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to retrieve mission' }), 
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
  });
  ...
  app.patch('/api/v1/missions/:id/progress', async (request: Request, params: { id: string }) => {
    try {
      const { id } = params;
      const body = await request.json();
      const { agentId, progress, message } = body as any;
      ...
      if (!agentId || typeof agentId !== 'string' || agentId.trim().length === 0) {
        return new Response(
          JSON.stringify({ error: 'Agent ID is required and must be a non-empty string' }), 
          { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
      }
  ...
  app.get('/api/v1/metrics', async (request: Request) => {
    try {
      const metrics = await progressTracker.getProgressMetrics();
