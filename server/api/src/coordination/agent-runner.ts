...
class AgentRunner {
   ...
   */
  private async runDefaultBehavior(): Promise<void> {
   ...
   * Send progress update
   */
  private async sendProgressUpdate(message: string, progress: number): Promise<void> {
    try {
   ...
   */
  private getDefaultActivities(): string[] {
    switch (this.config.agentType) {
      case 'frontend':
        return [
  ...
  private async initializeTestingAgent(): Promise<void> {
  ...
  private async initializeSecurityAgent(): Promise<void> {
    console.log('🔒 Initializing security environment');
    // Simulate security-specific setup
    await this.simulateWork(1000, 3000);
  }

  private async initializePerformanceAgent(): Promise<void> {
