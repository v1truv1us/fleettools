...
class CheckpointManager {
  ...
  async createCheckpoint(checkpoint: Omit<Checkpoint, 'id'>): Promise<Checkpoint> {
    ...
    try {
      ...
      stmt.run(
        ...
        checkpoint.trigger,
        checkpoint.trigger_details || null,
        checkpoint.progress_percent || null,
        JSON.stringify(checkpoint.sorties || []),
        JSON.stringify(checkpoint.active_locks || []),
        JSON.stringify(checkpoint.pending_messages || []),
        JSON.stringify(checkpoint.recovery_context || {}),
  ...
  async getCheckpointsByMission(missionId: string): Promise<Checkpoint[]> {
    try {
...
export function registerCheckpointRoutes(router: any, headers: Record<string, string>): void {
  // Create checkpoint
  router.post('/api/v1/checkpoints', async (request: Request) => {
    try {
  ...
  router.get('/api/v1/checkpoints/:id', async (request: Request, params: { id: string }) => {
    try {
  ...
  router.get('/api/v1/checkpoints', async (request: Request) => {
    try {
  ...
  router.get('/api/v1/checkpoints/latest/:missionId', async (request: Request, params: { missionId: string }) => {
    try {
  ...
  // Delete checkpoint
  router.delete('/api/v1/checkpoints/:id', async (request: Request, params: { id: string }) => {
    try {
  ...
  router.post('/api/v1/checkpoints/:id/resume', async (request: Request, params: { id: string }) => {
    try {
      const body = await request.json() as any;
