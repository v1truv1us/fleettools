...
export class ProgressTracker {
    ...
    async startMission(mission) {
      ...
      `);
            stmt.run(missionId, mission.title, mission.description, MissionStatus.PLANNED, 0.0, JSON.stringify(mission.tasks || []), now, now, JSON.stringify(mission.metadata || {}));
    ...
    async updateProgress(update) {
      ...
      `);
            updateStmt.run(updateId, update.missionId, update.agentId, update.progress, update.message || null, now);
     ...
     */
    async recalculateMissionProgress(missionId) {
        try {
    ...
    async completeMission(missionId) {
        ...
        WHERE id = ?
      `);
            stmt.run(MissionStatus.COMPLETED, new Date().toISOString(), new Date().toISOString(), missionId);
     ...
     */
    async getProgressHistory(missionId, limit = 100) {
        try {
            const stmt = this.db.prepare(`
    ...
    async getProgressMetrics() {
        try {
            ...
            const active = activeStmt.get(MissionStatus.ACTIVE, MissionStatus.PLANNED).count;
            const completed = completedStmt.get(MissionStatus.COMPLETED).count;
            const averageProgress = avgProgressStmt.get(MissionStatus.COMPLETED).avg || 0;
            return {
                totalMissions: total,
