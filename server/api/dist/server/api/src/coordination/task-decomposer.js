...
export class TaskDecomposer {
     ...
     */
    inferMissionType(title, description) {
     ...
     */
    createTaskFromPattern(template, mission) {
        const now = new Date().toISOString();
        const missionId = mission.metadata?.missionId || `mission_${Date.now()}`;
        return {
            id: `tsk_${Math.random().toString(36).substr(2, 9)}`,
    ...
    addTaskDependencies(tasks) {
        ...
        implementationTasks.forEach(implTask => {
            const matchingAnalysis = analysisTasks.find(analTask => implTask.metadata?.missionTitle === analTask.metadata?.missionTitle);
            if (matchingAnalysis) {
        ...
        testingTasks.forEach(testTask => {
            const matchingImpl = implementationTasks.find(implTask => testTask.metadata?.missionTitle === implTask.metadata?.missionTitle);
            if (matchingImpl) {
