...
import { createDatabaseClient } from '@fleettools/db/client.js';
export class TaskDispatcherService {
     ...
     */
    async assignWorkOrder(workOrderId, workType, description, priority = 'medium', preferredAgentType, context) {
        const availableAgents = await this.findBestAgents(workType, description, priority, preferredAgentType);
    ...
    `);
        stmt.run(assignment.id, assignment.agent_id, assignment.agent_callsign, assignment.work_order_id, assignment.work_type, assignment.priority, Math.floor(new Date(assignment.assigned_at).getTime() / 1000), assignment.status, assignment.context ? JSON.stringify(assignment.context) : null, assignment.progress_percent);
     ...
     */
    async findBestAgents(workType, description, priority, preferredAgentType) {
        ...
        const capableAgents = mockAgents.filter(agent => agent.status === 'idle' &&
     ...
     */
    canHandleWork(agent, workType, description) {
        const keywords = this.extractKeywords(workType + ' ' + description);
        return agent.capabilities.some(capability => capability.trigger_words.some(trigger => keywords.some(keyword => keyword.toLowerCase().includes(trigger.toLowerCase()))));
    }
    /**
     * Calculate agent fitness score
     */
    calculateAgentScore(agent, workType, description, priority) {
     ...
     */
    calculateCapabilityScore(agent, workType, description) {
        const keywords = this.extractKeywords(workType + ' ' + description);
        let bestScore = 0;
     ...
     */
    async updateAssignmentStatus(assignmentId, status, progressPercent, errorDetails) {
    ...
    `);
        const rows = stmt.all(...activeStatuses);
        return rows.map(row => ({
            id: row.id,
            agent_id: row.agent_id,
            agent_callsign: row.agent_callsign,
