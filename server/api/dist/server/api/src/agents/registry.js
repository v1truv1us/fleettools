...
export class AgentRegistryService {
    ...
    initializeAgentCapabilities() {
        ...
        this.agentCapabilities = new Map(Object.entries(capabilities));
    }
    /**
    ...
    `);
        stmt.run(agent.id, agent.agent_type, agent.callsign, agent.status, JSON.stringify(agent.capabilities), agent.current_workload, agent.max_workload, Math.floor(new Date(agent.last_heartbeat).getTime() / 1000), agent.metadata ? JSON.stringify(agent.metadata) : null, Math.floor(new Date(agent.created_at).getTime() / 1000), Math.floor(new Date(agent.updated_at).getTime() / 1000));
     ...
     */
    async getAgentByCallsign(callsign) {
        const sqlite = this.db.$client;
    ...
    async getAvailableAgents(agentType) {
      ...
      `);
        ...
        const rows = stmt.all(agentType || null);
        return rows.map(row => ({
            id: row.id,
            agent_type: row.agent_type,
            callsign: row.callsign,
     ...
     */
    async updateAgentWorkload(id, workload) {
        const now = Math.floor(Date.now() / 1000);
        const sqlite = this.db.$client;
        const stmt = sqlite.prepare(`
      UPDATE agents 
     ...
     */
    async findAgentsByCapability(triggerWord) {
     ...
     */
    async initializeAgentHealth(agentId, callsign) {
        const now = Math.floor(Date.now() / 1000);
        const sqlite = this.db.$client;
        const stmt = sqlite.prepare(`
    ...
    `);
        stmt.run(randomUUID(), agentId, 'offline', now, false, true, true, true, true, now, now);
    }
    /**
     * Get stale agents (missed heartbeats)
     */
    async getStaleAgents(timeoutMs = 180000) {
