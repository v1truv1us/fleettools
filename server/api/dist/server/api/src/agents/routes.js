...
export function registerAgentRoutes(router, headers) {
    ...
    router.post('/api/v1/assignments', async (req) => {
        try {
            const body = await req.json();
            // Find suitable agent
            const suitableAgent = Array.from(agents.values()).find(agent => agent.status === 'idle' &&
                agent.current_workload < agent.max_workload &&
                agent.capabilities.some(cap => cap.trigger_words.some(trigger => body.work_type.toLowerCase().includes(trigger.toLowerCase()))));
            if (!suitableAgent) {
                return new Response(JSON.stringify({
