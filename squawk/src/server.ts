/**
 * Squawk Server Module
 * 
 * Contains the server startup logic that can be imported and used programmatically.
 * This separates the server logic from library exports to avoid side effects.
 */

import { initializeDatabase, closeDatabase, mailboxOps, eventOps, cursorOps, lockOps } from './db/index.js';

export interface SquawkServerOptions {
  port?: number;
...
/**
 * Start the Squawk server with the given options
 */
export async function startSquawkServer(opts: SquawkServerOptions = {}): Promise<void> {
  const port = opts.port || parseInt(process.env.SQUAWK_PORT || '3000', 10);
