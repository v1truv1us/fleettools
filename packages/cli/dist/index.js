    ...
    static from(schema, obj, ctx) {
      ...
      const add = (key, value) => {
        ...
        if (value !== undefined || keepUndefined)
          map.items.push(Pair2.createPair(key, value, ctx));
      };
...
var require_bool5 = __commonJS((exports) => {
  var Scalar2 = require_Scalar2();
...
var require_float5 = __commonJS((exports) => {
  var Scalar2 = require_Scalar2();
  var stringifyNumber = require_stringifyNumber2();
  var floatNaN = {
    identify: (value) => typeof value === "number",
...
var require_int5 = __commonJS((exports) => {
  var stringifyNumber = require_stringifyNumber2();
  var intIdentify = (value) => typeof value === "bigint" || Number.isInteger(value);
...
var require_schema7 = __commonJS((exports) => {
  var map = require_map2();
  var _null = require_null2();
  var seq = require_seq2();
  var string = require_string2();
  var binary = require_binary2();
  var bool = require_bool5();
  var float = require_float5();
  var int = require_int5();
  var merge = require_merge2();
...
var require_tags2 = __commonJS((exports) => {
  ...
  var schema$1 = require_schema6();
  var binary = require_binary2();
  var merge = require_merge2();
  var omap = require_omap2();
  var pairs = require_pairs2();
  var schema$2 = require_schema7();
  var set = require_set2();
  var timestamp = require_timestamp2();
  var schemas = new Map([
    ["core", schema.schema],
...
function initializeProject(projectPath, templateName, config = {}) {
  const template = PROJECT_TEMPLATES[templateName];
  if (!template) {
    throw new Error(`Unknown template: ${templateName}`);
  }
  template.directories.forEach((dir) => {
    const fullPath = join2(projectPath, dir);
...
function registerInitCommand(program2) {
  program2.command("init").description("Initialize a new FleetTools project").option("-t, --template <template>", "Project template to use", "basic").option("-n, --name <name>", "Project name").option("-y, --yes", "Accept all defaults").argument("[path]", "Directory to initialize (default: current directory)", ".").action(async (projectPath, options) => {
    try {
      ...
      const projectDir = join5(process.cwd(), projectPath);
      console.log(source_default.blue("\uD83D\uDCE6 Installing FleetTools dependencies..."));
      try {
        const pkgJsonPath = join5(projectDir, "package.json");
        ...
        const installCmd = pkgManager === "bun" ? "bun add @fleettools/server @fleettools/squawk" : "npm install @fleettools/server @fleettools/squawk";
        execSync(installCmd, {
          cwd: projectDir,
          stdio: "inherit"
        });
        ...
        console.error(source_default.yellow("⚠️  Failed to auto-install dependencies."));
        console.log(source_default.yellow("Run manually: bun add @fleettools/server @fleettools/squawk"));
      }
      console.log();
      ...
      console.log();
      console.log(source_default.blue("Available commands:"));
...
\uD83D\uDED1 Received ${signal}, stopping services...`));
            processes.forEach(({ process: process3, name }) => {
              if (process3 && !process3.killed) {
                console.log(source_default.blue(`Stopping ${name}...`));
          ...
          if (options.watch) {
            console.log(source_default.blue("\uD83D\uDC40 Watching for changes..."));
          }
        } else {
          console.log(source_default.gray("Services are running in the background."));
          console.log(source_default.yellow("Use: fleet status to check service status"));
...
function registerConfigCommand(program2) {
  const configCmd = program2.command("config").description("Manage FleetTools configuration");
...
function registerStatusCommand(program2) {
  program2.command("status").description("Show FleetTools status and configuration").option("--json", "Output in JSON format").action(async (options) => {
    try {
      const runtimeInfo = getRuntimeInfo();
      const globalConfig = loadGlobalConfig();
      const statusData = {
...
import { existsSync as existsSync7, mkdirSync as mkdirSync4, promises as fs } from "node:fs";
...
function registerDoctorCommand(program2) {
  program2.command("doctor").description("Diagnose FleetTools installation and configuration").option("--json", "Output in JSON format").option("--fix", "Attempt to fix common issues automatically").action(async (options) => {
    try {
      ...
      for (const tool of tools) {
        ...
        try {
          if (available) {
            ...
            const version = execSync2(`${tool.name} --version`, { encoding: "utf-8" }).trim();
      ...
      if (!existsSync8(globalConfig.paths.configDir)) {
...
import { appendFileSync, existsSync as existsSync9, mkdirSync as mkdirSync6, readFileSync as readFileSync6 } from "node:fs";
...
import { dirname as dirname2, join as join11, resolve as resolve5, sep } from "node:path";
...
class StateCorruptionError extends SteerError {
...
async function discoverProjectRoot(start = process.cwd()) {
  let current = resolve5(start);
  while (true) {
    if (await pathExists(join11(current, ".git")))
      return current;
  ...
  current = resolve5(start);
  while (true) {
    if (await pathExists(join11(current, "package.json")))
      return current;
    const parent = dirname2(current);
    if (parent === current)
      return resolve5(start);
    current = parent;
  }
}
function changeDir(root, slug) {
  validateSlug(slug);
  return join11(root, "specs", slug);
}
async function inferSlug(root, explicit) {
  if (explicit) {
    validateSlug(explicit);
    return explicit;
  }
  const cwd = resolve5(process.cwd());
  const specsRoot = join11(resolve5(root), "specs") + sep;
...
async function writeTextAtomic(filePath, content) {
  await fs2.mkdir(dirname2(filePath), { recursive: true });
  const tmp = join11(dirname2(filePath), `.${filePath.split("/").pop()}.${process.pid}.${randomUUID()}.tmp`);
...
async function readTextIfExists(path) {
  if (!await pathExists(path))
    return "";
  return fs2.readFile(path, "utf8");
}
async function assertNonEmptyFile(path, label) {
  const text = await readTextIfExists(path);
  if (text.trim().length === 0)
    throw new SteerError(`${label} is missing or empty: ${path}`);
}
async function withChangeLock(dir, fn) {
  const lockDir = join11(dir, ".steer-lock");
  const recoveryLockDir = join11(dir, ".steer-lock-recovery");
  const acquire = async (duringRecovery = false) => {
    if (!duringRecovery && await pathExists(recoveryLockDir))
      throw new Error("lock recovery in progress");
    await fs2.mkdir(lockDir, { recursive: false });
    await fs2.writeFile(join11(lockDir, "owner.json"), JSON.stringify({ pid: process.pid, at: new Date().toISOString() }));
  };
  try {
    await acquire();
  } catch {
...
async function recoverStaleLock(lockDir, recoveryLockDir, changeDir2, acquire) {
  try {
    await fs2.mkdir(recoveryLockDir, { recursive: false });
  } catch {
    throw new SteerError(`Another steer operation is running for this change: ${changeDir2}`);
  }
  try {
    ...
    await fs2.rm(recoveryLockDir, { recursive: true, force: true });
  }
}
async function isStaleLock(lockDir) {
  try {
    const owner = JSON.parse(await fs2.readFile(join11(lockDir, "owner.json"), "utf8"));
    if (owner.pid && isPidAlive3(owner.pid))
      return false;
...
// src/steer/state.ts
var phases = new Set(["context", "spec", "spec-review", "plan", "plan-review", "tasks", "execute", "verify", "done"]);
function validateState(value, path) {
  if (!value || typeof value !== "object")
    throw new StateCorruptionError(path, "expected object");
  const state = value;
  if (typeof state.phase !== "string" || !phases.has(state.phase)) {
    throw new StateCorruptionError(path, "invalid phase");
  }
  if (typeof state.round !== "number" || state.round < 0)
    throw new StateCorruptionError(path, "invalid round");
  if (!Array.isArray(state.tasks))
    throw new StateCorruptionError(path, "tasks must be an array");
...
async function readState(dir) {
  const path = join12(dir, "state.json");
  let raw;
  try {
...
async function writeState(dir, state) {
  await writeJsonAtomic(join12(dir, "state.json"), validateState(state, join12(dir, "state.json")));
}
function appendHistory(state, action, detail) {
  return { ...state, history: [...state.history, { at: new Date().toISOString(), action, detail }] };
}

// src/steer/templates.ts
import { promises as fs4 } from "node:fs";
import { dirname as dirname3, join as join13 } from "node:path";
import { fileURLToPath } from "node:url";
async function findCliPackageRoot() {
  ...
  while (true) {
    ...
    if (parent === current)
      throw new SteerError("Could not locate @fleettools/cli package root for templates.");
    current = parent;
  }
}
async function loadTemplate(name, vars, projectRoot = process.cwd()) {
  const override = join13(projectRoot, ".steer", "templates", `${name}.md`);
  const packageRoot = await findCliPackageRoot();
  const packaged = join13(packageRoot, "templates", "steer", `${name}.md`);
  const templatePath = await pathExists(override) ? override : packaged;
  if (!await pathExists(templatePath))
    throw new SteerError(`Missing steer template: ${templatePath}`);
  const source = await fs4.readFile(templatePath, "utf8");
  return source.replace(/{{\s*([a-zA-Z0-9_]+)\s*}}/g, (_match, key) => {
    if (!(key in vars))
      throw new SteerError(`Missing template variable "${key}" for ${templatePath}`);
    return String(vars[key]);
  });
}

// src/steer/verdict.ts
...
function parseVerdict(raw) {
  try {
    ...
    if (keys !== "issues,lens,round,status")
      return null;
    if (value.status !== "pass" && value.status !== "issues")
      return null;
    if (!Number.isInteger(value.round) || value.round < 1)
      return null;
    if (!["ambiguity", "security-perf", "maintainability", "custom"].includes(value.lens))
      return null;
    if (!Array.isArray(value.issues))
      return null;
    for (const issue of value.issues) {
      const issueKeys = Object.keys(issue).sort().join(",");
      if (issueKeys !== "description,severity" && issueKeys !== "description,location,severity")
        return null;
      if (!["high", "medium", "low"].includes(issue.severity) || typeof issue.description !== "string")
        return null;
      if ("location" in issue && typeof issue.location !== "string")
        return null;
    }
    return value;
  } catch {
...
async function loadSteerConfig(projectRoot = process.cwd()) {
  const candidates = [join14(projectRoot, ".steer", "config.yaml"), join14(homedir2(), ".config", "fleet", "steer.yaml")];
  let config = defaultConfig;
  for (const candidate of candidates) {
    if (await pathExists(candidate)) {
      const parsed = import_yaml.default.parse(await fs5.readFile(candidate, "utf8")) ?? {};
...
async function parseTasksFile(changeDir2) {
  const raw = await readTextIfExists(join15(changeDir2, "05-tasks.md"));
...
async function readTaskStatus(changeDir2, taskId) {
  return JSON.parse(await fs6.readFile(join15(changeDir2, "tasks", taskId, "status.json"), "utf8"));
}
async function writeTaskStatus(changeDir2, status) {
  await writeJsonAtomic(join15(changeDir2, "tasks", status.id, "status.json"), status);
}
async function scaffoldTasks(changeDir2, projectRoot) {
  const tasks = await parseTasksFile(changeDir2);
  for (const task of tasks) {
    const dir = join15(changeDir2, "tasks", task.id);
    await fs6.mkdir(dir, { recursive: true });
    if (!await pathExists(join15(dir, "prompt.md"))) {
      await writeTextAtomic(join15(dir, "prompt.md"), await loadTemplate("task-prompt", { task_id: task.id, task_description: task.description, deps: task.deps.join(", ") || "none" }, projectRoot));
    }
    if (!await pathExists(join15(dir, "verify.md"))) {
      await writeTextAtomic(join15(dir, "verify.md"), await loadTemplate("task-verify", { task_id: task.id, task_description: task.description }, projectRoot));
    }
    if (!await pathExists(join15(dir, "status.json"))) {
      await writeTaskStatus(changeDir2, { id: task.id, status: "pending", deps: task.deps, description: task.description });
    }
  }
  return tasks;
}
async function listTaskStatuses(changeDir2) {
  const tasks = await parseTasksFile(changeDir2);
  const statuses = [];
  for (const task of tasks) {
    const statusPath = join15(changeDir2, "tasks", task.id, "status.json");
    if (await pathExists(statusPath))
      statuses.push(await readTaskStatus(changeDir2, task.id));
    else
      statuses.push({ id: task.id, status: "pending", deps: task.deps, description: task.description });
  }
  return statuses;
}
async function getActiveTask(changeDir2) {
  return (await listTaskStatuses(changeDir2)).find((task) => task.status === "active" || task.status === "verifying");
}
function depsSatisfied(task, all) {
  return task.deps.every((dep) => all.some((candidate) => candidate.id === dep && candidate.status === "done"));
}

// src/steer/handoff.ts
import { basename, dirname as dirname4, join as join16, relative } from "node:path";
async function mockHandoff(changeDir2, task) {
  if (process.env.STEER_MOCK_HANDOFF_FAIL === "1")
    throw new Error("Mock handoff failure requested by STEER_MOCK_HANDOFF_FAIL=1");
  const id = `mock-solo-${task.id}-${Date.now()}`;
  await writeJsonAtomic(join16(changeDir2, "tasks", task.id, "handoff.json"), {
    id,
    task_id: task.id,
    description: task.description,
...
async function createSoloTaskHandoff(changeDir2, task, projectRoot = dirname4(dirname4(changeDir2))) {
  if (task.solo_task_id)
    return { id: task.solo_task_id, status: task.solo_status };
  if (process.env.STEER_SOLO_MOCK === "1")
    return { id: await mockHandoff(changeDir2, task), status: "mock" };
  const input = await buildSoloTaskInput(changeDir2, task, projectRoot);
  ...
  await writeHandoffArtifact(changeDir2, task, created.id, created.status, projectRoot);
  return { id: created.id, status: created.status };
}
async function completeHandoff(changeDir2, taskId, projectRoot) {
  const task = await readTaskStatus(changeDir2, taskId);
  const handoff = await createSoloTaskHandoff(changeDir2, task, projectRoot);
  const done = {
    ...task,
    status: "done",
  ...
  await writeTaskStatus(changeDir2, done);
  return done;
}
async function buildSoloTaskInput(changeDir2, task, projectRoot) {
  const slug = basename(changeDir2);
  const verifyText = await readTextIfExists(join16(changeDir2, "tasks", task.id, "verify.md"));
  const verifyResultPath = join16(changeDir2, "tasks", task.id, "verify-result.md");
  const verifyResultRelative = relative(projectRoot, verifyResultPath);
  const statuses = await listTaskStatuses(changeDir2);
  const deps = task.deps.map((dep) => statuses.find((candidate) => candidate.id === dep)?.solo_task_id).filter((dep) => typeof dep === "string" && dep.length > 0);
  return {
...
async function writeHandoffArtifact(changeDir2, task, soloTaskId, soloStatus, projectRoot) {
  const verifyResult = relative(projectRoot, join16(changeDir2, "tasks", task.id, "verify-result.md"));
  await writeJsonAtomic(join16(changeDir2, "tasks", task.id, "handoff.json"), {
    id: soloTaskId,
    solo_task_id: soloTaskId,
    solo_status: soloStatus,
    task_id: task.id,
    source_change: basename(changeDir2),
...
function extractDeclaredFiles(verifyText) {
  const files = [];
  let inFiles = false;
  for (const line of verifyText.split(`
`)) {
    if (/^#+\s+Files (created|modified)/i.test(line)) {
      inFiles = true;
      continue;
    }
    if (/^#+\s+/.test(line))
    ...
    if (inFiles) {
      ...
      if (match && !match[1].includes("path/to/"))
        files.push(match[1]);
    }
  }
  return [...new Set(files)];
}
function extractAcceptanceCriteria(verifyText) {
...
async function runShell(command, cwd, timeoutSeconds) {
  return new Promise((resolve4) => {
    const child = spawn3(command, { cwd, shell: true, stdio: ["ignore", "pipe", "pipe"] });
    let output = "";
    const timer = setTimeout(() => {
      child.kill("SIGTERM");
      resolve4({ ok: false, output: `${output}
...
function parseDeclaredFiles(verifyText) {
  const files = [];
  let inFiles = false;
  for (const line of verifyText.split(`
`)) {
...
function parseShellCriteria(verifyText) {
  const noComments = verifyText.replace(/<!--[\s\S]*?-->/g, "");
  return [...noComments.matchAll(/\[shell:\s*([^\]]+)\]/g)].map((match) => match[1].trim());
}
async function gitDiff(projectRoot, baseRef) {
  const command = baseRef ? `git diff ${baseRef}` : "git diff";
  return (await runShell(command, projectRoot, 30)).output;
}
async function runDeterministicVerify(changeDir2, projectRoot, testsEnabled) {
  const active = await getActiveTask(changeDir2);
  if (!active)
    throw new SteerError("No active task to verify. Run fleet steer task start first.");
  const config = await loadSteerConfig(projectRoot);
  const verifyText = await fs7.readFile(join17(changeDir2, "tasks", active.id, "verify.md"), "utf8");
  const sections = [`# Verify Result: ${active.id}`, `Started: ${new Date().toISOString()}`];
...
\`\`\``);
    if (!result.ok)
      return { ok: false, markdown: sections.join(`
...
Skipped by configuration or --no-tests.`);
  ...
  for (const file of declared) {
    try {
      const stat = await fs7.stat(join17(projectRoot, file));
      if (stat.size === 0)
...
async function emitDiffReview(changeDir2, projectRoot, testsEnabled) {
  const result = await runDeterministicVerify(changeDir2, projectRoot, testsEnabled);
  await writeTextAtomic(join17(changeDir2, "tasks", result.taskId, "verify-result.md"), result.markdown);
  const task = await readTaskStatus(changeDir2, result.taskId);
  if (!result.ok) {
    await writeTaskStatus(changeDir2, { ...task, status: "failed" });
    throw new SteerError(`Deterministic verify failed for ${result.taskId}. See tasks/${result.taskId}/verify-result.md`);
  }
  await writeTaskStatus(changeDir2, { ...task, status: "verifying" });
  const state = await readState(changeDir2);
  await writeState(changeDir2, appendHistory({ ...state, phase: "verify", active_task: result.taskId }, "verify", `deterministic checks passed for ${result.taskId}`));
  const spec = await readTextIfExists(join17(changeDir2, "01-spec.md"));
  const verifyText = await readTextIfExists(join17(changeDir2, "tasks", result.taskId, "verify.md"));
...
async function ingestDiffReview(changeDir2, raw) {
  const active = await getActiveTask(changeDir2);
  if (!active || active.status !== "verifying")
    throw new SteerError("No task is awaiting diff-review ingestion.");
  const verdict = parseVerdict(raw);
  if (!verdict) {
    await writeTextAtomic(join17(changeDir2, "tasks", active.id, "diff-review-raw.txt"), raw);
    throw new SteerError(`Invalid strict verdict JSON. Raw output saved for ${active.id}.`);
  }
  const existing = await readTextIfExists(join17(changeDir2, "tasks", active.id, "verify-result.md"));
...
\`\`\`
`;
  await writeTextAtomic(join17(changeDir2, "tasks", active.id, "verify-result.md"), final);
  const state = await readState(changeDir2);
  if (verdict.status !== "pass") {
    await writeTaskStatus(changeDir2, { ...active, status: "failed" });
    await writeState(changeDir2, appendHistory({ ...state, active_task: active.id }, "verify-failed", active.id));
  ...
  let soloStatus;
  try {
    const handoff = await createSoloTaskHandoff(changeDir2, active);
    handoffId = handoff.id;
    soloStatus = handoff.status;
  } catch {
    nextStatus = "done-pending-handoff";
  }
  await writeTaskStatus(changeDir2, {
    ...active,
    status: nextStatus,
  ...
  await writeState(changeDir2, appendHistory({ ...state, phase: allComplete ? "done" : "execute", active_task: undefined }, "verify-passed", active.id));
...
// src/commands/steer.ts
var stdout = (text) => process.stdout.write(text.endsWith(`
`) ? text : `${text}
`);
var stderr = (text) => process.stderr.write(text.endsWith(`
`) ? text : `${text}
`);
async function context(explicit) {
...
function artifactFor(phase) {
  switch (phase) {
    case "context":
      return "00-context.md";
    case "spec":
      return "01-spec.md";
    case "plan":
      return "03-plan.md";
...
function priorArtifactFor(phase) {
  switch (phase) {
    case "spec":
      return "00-context.md";
    case "plan":
    ...
    case "verify":
    case "done":
      return artifactFor(phase) ?? "05-tasks.md";
  }
}
function nextPhase(phase) {
  switch (phase) {
    ...
    case "spec-review":
    case "plan-review":
      throw new SteerError("Use fleet steer review --ingest or --resolve to leave review phases.");
    case "execute":
      throw new SteerError("Use fleet steer task start and fleet steer verify for execute/verify phases.");
    case "verify":
      throw new SteerError("Use fleet steer verify --ingest-review to complete verification.");
    case "done":
      throw new SteerError("Change is already done.");
  }
}
async function promptFor(root, dir, slug) {
  const state = await readState(dir);
  if (state.phase === "spec-review" || state.phase === "plan-review")
    return reviewPrompt(root, dir, slug);
  const templateName = state.phase === "execute" ? "execute" : state.phase;
  const prior = await readTextIfExists(join18(dir, priorArtifactFor(state.phase)));
  return loadTemplate(templateName, { slug, phase: state.phase, round: state.round, prior_artifact: prior }, root);
}
async function reviewPrompt(root, dir, slug) {
  const state = await readState(dir);
  if (state.phase !== "spec-review" && state.phase !== "plan-review")
    throw new SteerError("Review prompts are only available during spec-review or plan-review.");
  const round = state.round + 1;
  const lensRound = Math.min(round, 3);
  const artifact = await readTextIfExists(join18(dir, state.phase === "spec-review" ? "01-spec.md" : "03-plan.md"));
  const contextText = await readTextIfExists(join18(dir, "00-context.md"));
  return loadTemplate(`review-r${lensRound}`, { slug, round, phase: state.phase, artifact, context: contextText }, root);
}
async function initSteer(slug) {
  validateSlug(slug);
  const root = await discoverProjectRoot();
  const dir = changeDir(root, slug);
  if (await pathExists(dir))
    throw new SteerError(`Change already exists: ${dir}`);
  await fs8.mkdir(join18(dir, "02-spec-reviews"), { recursive: true });
  await fs8.mkdir(join18(dir, "04-plan-reviews"), { recursive: true });
  await fs8.mkdir(join18(dir, "tasks"), { recursive: true });
  await writeTextAtomic(join18(dir, "00-context.md"), "");
  await writeTextAtomic(join18(dir, "01-spec.md"), "");
  await writeTextAtomic(join18(dir, "03-plan.md"), "");
  await writeTextAtomic(join18(dir, "05-tasks.md"), "");
  await writeTextAtomic(join18(dir, "decisions.md"), `# Decisions: ${slug}

`);
  await writeJsonAtomic(join18(dir, "state.json"), defaultState());
  stderr(`Initialized steer change at ${dir}`);
}
async function advanceSteer(slug) {
  const { root, dir } = await context(slug);
  await withChangeLock(dir, async () => {
    const state = await readState(dir);
    const artifact = artifactFor(state.phase);
    if (artifact)
      await assertNonEmptyFile(join18(dir, artifact), artifact);
    if (state.phase === "tasks") {
      const tasks = await scaffoldTasks(dir, root);
      if (tasks.length === 0)
        throw new SteerError("05-tasks.md contains no markdown checklist tasks.");
...
async function statusSteer(slug, json = false) {
  const { dir, slug: resolved } = await context(slug);
  const state = await readState(dir);
  if (json) {
    stdout(JSON.stringify({ slug: resolved, ...state }, null, 2));
    return;
  }
...
async function ingestReview(slug) {
  const { dir } = await context(slug);
  await withChangeLock(dir, async () => {
    const state = await readState(dir);
    if (state.phase !== "spec-review" && state.phase !== "plan-review")
      throw new SteerError("Not in a review phase.");
    const raw = await readStdin();
    const verdict = parseVerdict(raw);
    const round = state.round + 1;
    const reviewDir = join18(dir, state.phase === "spec-review" ? "02-spec-reviews" : "04-plan-reviews");
    if (!verdict) {
      await writeTextAtomic(join18(reviewDir, `round-${round}-raw.txt`), raw);
      throw new SteerError(`Invalid strict verdict JSON. Raw output saved to ${reviewDir}/round-${round}-raw.txt`);
    }
    await writeTextAtomic(join18(reviewDir, `round-${round}.md`), `\`\`\`json
${JSON.stringify(verdict, null, 2)}
\`\`\`
`);
    await writeTextAtomic(join18(dir, "decisions.md"), `${await readTextIfExists(join18(dir, "decisions.md"))}
...
`);
    ...
    if (passes >= config.review.early_exit_on_pass_count) {
      const next = state.phase === "spec-review" ? "plan" : "tasks";
...
async function resolveReview(slug) {
  const { dir } = await context(slug);
  await withChangeLock(dir, async () => {
    const state = await readState(dir);
    if (state.phase !== "spec-review" && state.phase !== "plan-review")
...
async function taskStart(slug) {
  ...
  await withChangeLock(dir, async () => {
    ...
    if (statuses.some((task) => task.status === "active" || task.status === "verifying"))
      throw new SteerError("A task is already active or verifying.");
    const next = statuses.find((task) => task.status === "pending" && depsSatisfied(task, statuses));
    if (!next)
      throw new SteerError("No ready pending task found.");
    ...
    stdout(await fs8.readFile(join18(dir, "tasks", next.id, "prompt.md"), "utf8"));
  });
}
async function taskRetry(taskId, slug) {
  const { dir } = await context(slug);
  await withChangeLock(dir, async () => {
    const state = await readState(dir);
    if (state.phase !== "execute" && state.phase !== "verify")
      throw new SteerError(`Cannot retry tasks while change is in ${state.phase} phase.`);
    const statuses = await listTaskStatuses(dir);
    if (statuses.some((task2) => (task2.status === "active" || task2.status === "verifying") && task2.id !== taskId))
      throw new SteerError("Another task is already active or verifying.");
    const task = await readTaskStatus(dir, taskId);
    if (task.status !== "failed")
      throw new SteerError(`Task ${taskId} is ${task.status}; only failed tasks can be retried.`);
...
async function verifySteer(slug, opts) {
  const { root, dir } = await context(slug);
  await withChangeLock(dir, async () => {
    const state = await readState(dir);
    if (opts.ingestReview) {
      if (state.phase !== "verify")
        throw new SteerError(`Cannot ingest diff review while change is in ${state.phase} phase.`);
      stdout(await ingestDiffReview(dir, await readStdin()));
      return;
    }
    if (state.phase !== "execute")
      throw new SteerError(`Cannot verify while change is in ${state.phase} phase.`);
    stdout(await emitDiffReview(dir, root, opts.tests));
  });
}
function registerSteerCommand(program2) {
  const steer = program2.command("steer").description("Spec-driven Fleet Steer authoring workflow");
  const act = (fn) => (...args) => fn(...args).catch((error) => {
    stderr(error instanceof Error ? error.message : String(error));
    process.exit(error instanceof SteerError ? error.exitCode : 1);
  });
  ...
  steer.command("next [slug]").description("Emit current phase prompt").action(act(async (slug) => {
    const c = await context(slug);
    stdout(await promptFor(c.root, c.dir, c.slug));
  }));
  steer.command("resume [slug]").description("Alias for next").action(act(async (slug) => {
    const c = await context(slug);
    stdout(await promptFor(c.root, c.dir, c.slug));
  }));
  steer.command("advance [slug]").description("Validate artifact and advance phase").action(act(advanceSteer));
  steer.command("review [slug]").option("--ingest", "Read strict verdict JSON from stdin").option("--resolve", "Resolve review and advance").action(act(async (slug, opts) => {
    if (opts.ingest)
      return ingestReview(slug);
    if (opts.resolve)
      return resolveReview(slug);
    const c = await context(slug);
    stdout(await reviewPrompt(c.root, c.dir, c.slug));
  }));
  const task = steer.command("task").description("Manage steer tasks");
  task.command("list [slug]").action(act(async (slug) => {
    const { dir } = await context(slug);
    await scaffoldTasks(dir, await discoverProjectRoot());
    const rows = await listTaskStatuses(dir);
    stdout(rows.map((row) => `${row.id}	${row.status}	deps:${row.deps.join(",") || "-"}	${row.description}`).join(`
`) || "No tasks.");
  }));
  ...
  steer.command("handoff [slug]").option("--retry", "Retry Solo handoff for pending tasks").action(act(async (slug, opts) => {
    if (!opts.retry)
      throw new SteerError("Only fleet steer handoff --retry is supported in the MVP.");
    const { root, dir } = await context(slug);
    await withChangeLock(dir, async () => {
      const rows = await listTaskStatuses(dir);
      const pending = rows.filter((task2) => task2.status === "done-pending-handoff");
      for (const row of pending)
        await completeHandoff(dir, row.id, root);
      stdout(`Retried ${pending.length} pending handoff(s).`);
    });
  }));
  steer.command("rollback").description("Refuse dangerous rollback in MVP").action(act(async () => {
    throw new SteerError("Rollback is intentionally disabled in the MVP to avoid reverting unrelated work. Use git status/diff and revert manually.");
  }));
}
...
var pkg = JSON.parse(readFileSync7(resolve6(__dirname2, "../package.json"), "utf8"));
...
program.name("fleet").description("FleetTools - AI Agent Coordination System CLI").version(pkg.version).option("--config <path>", "Path to global config file").option("--verbose", "Enable verbose output").option("--debug-runtime", "Show runtime information and exit").hook("preAction", (thisCommand) => {
  const options = thisCommand.opts();
  if (options.verbose) {
    console.error(source_default.blue(`Running: ${thisCommand.name()}`));
    console.error(source_default.blue(`Runtime: ${runtime} (${runtimeInfo.version})`));
  }
});
...
program.on("command:*", () => {
  if (program.args.length === 0) {
    console.log(source_default.blue.bold("FleetTools CLI"));
