...
export default async function FleetToolsPlugin() {
  ...
  return {
    tool: {
      'fleet-status': {
        ...
        execute: async ({ format = 'text' }: { format?: string }) => {
      ...
      'fleet-start': {
        ...
        execute: async ({ services, foreground }: { services?: string; foreground?: boolean }) => {
