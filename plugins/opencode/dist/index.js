...
async function runFleet(args, cwd) {
  try {
    const shellApi = globalThis.$ || globalThis.bun?.shell;
    if (!shellApi) {
...
var FleetToolsPlugin = async ({ client, directory, worktree }) => {
  ...
  return {
    tool: {
      fleet_status: tool({
        ...
        args: {
          format: z2.string().optional().describe("Output format: text or json")
        },
      ...
      fleet_start: tool({
        ...
        args: {
          services: z2.string().optional().describe("Services to start (comma-separated): api,squawk")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
      ...
      fleet_stop: tool({
        ...
        args: {
          services: z2.string().optional().describe("Services to stop (comma-separated: api,squawk"),
          force: z2.boolean().optional().describe("Force stop without graceful shutdown"),
          timeout_ms: z2.number().optional().describe("Timeout for graceful shutdown in milliseconds"),
          format: z2.string().optional().describe("Output format: text or json")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
      ...
      fleet_setup: tool({
        ...
        args: {
          global: z2.boolean().optional().describe("Setup global configuration only"),
          force: z2.boolean().optional().describe("Force re-initialization")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
      ...
      fleet_doctor: tool({
        ...
        args: {
          json: z2.boolean().optional().describe("Output in JSON format"),
          fix: z2.boolean().optional().describe("Attempt to fix common issues automatically")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
      ...
      fleet_services: tool({
        ...
        args: {
          args: z2.string().optional().describe("Arguments to pass to fleet services command")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
      ...
      fleet_context: tool({
        ...
        args: {
          verbose: z2.boolean().optional().describe("Include detailed information")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
          try {
...
Error: ${error.message || String(error)}`;
      ...
      fleet_opencode_setup: {
        ...
        args: {
          project_path: z2.string().optional().describe("Project path (defaults to current directory)"),
          overwrite: z2.boolean().optional().describe("Overwrite existing command files")
        },
        async execute(args, context) {
          const cwd = context.worktree ?? context.directory;
...
/fleet help`;
          ...
          for (const cmd of commands) {
            ...
            const content = commandTemplate.replace(/\$1/g, `${cmd.name}-${cmd.commands}`).replace(/\$2/g, cmd.desc).replace(/\$commands/g, cmd.commands).replace(/\$ARGUMENTS/g, cmd.extra);
